...
def parse_profile():
    ...
    for item in experience_items:
        ...
        date_range = item.select_one(".pvs-entity__caption-wrapper span[aria-hidden='true']")
        ...
        if title is not None and company is not None:
            job_data.append({
                "Job Title": clean_text(title.get_text(strip=True)) if title else None,
                "Company": clean_text(company.get_text(strip=True)) if company else None,
                "Date Range": clean_text(date_range.get_text(strip=True)) if date_range else None,
                "Location": clean_text(ex_location.get_text(strip=True)) if ex_location else None,
    ...
    for li in soup.select("ul.CcPTLnqQqRzyebUepUmLiAtEiwlybuYmQSI > li"):
        ...
        activities = li.select_one(".inline-show-more-text--is-collapsed span[aria-hidden='true']")

        if school_name:
            sname = school_name.get_text(strip=True).lower()
            if any(keyword in sname for keyword in KEYWORDS):
                education_data.append({
                    "School": clean_text(school_name.get_text() if school_name else None),
                    ...
                    "Activities": clean_text(
                        activities.get_text().replace("Activities and societies:", "") if activities else None)
                })

    # Clean out None-only entries
    education_data = [entry for entry in education_data if any(v for v in entry.values())]
